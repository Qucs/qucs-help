...
Figure 2.4. A section of an ngspice execution log displayed in the *Simulate 
with an external simulator* dialog window.
...
of a 1N4148 diode. This example is ``examples\ngspice\diode.sch`` in examples 
tree of Qucs.
...
* For SPICE DC-sweep: specify a sweep source name or a resistor name NOT a 
source or resistor value, for example ``R1`` or  ``V1``, see ``V1`` in Figure 
2.11.
...
AC simulation is fully supported by *spice4qucs* subsystem. It doesn't require 
any special adaptation. Just simple place ``AC simulation`` component on 
schematic and execute Ngspice or Xyce simulation. For variable names conversion 
see Table 2.1. Qucs dataset builder adds for all variables obtained from AC 
simulation an ``ac.`` prefix. 

Frequency domain simulations don't use adaptive simulation step. 

2.6 TRAN
~~~~~~~~~~

Transient simulation is fully supported by ``spice4qucs`` subsystem too. Just 
place ``Transient simulation`` component on schematic and simulate it. There is 
a difference only in time step.

Qucsator always uses fixed time step. Ngspice and Xyce use adaptive time step. 
Simulation points amount will be only approximately equals of simulation points 
specified in ``Transient simulation`` component properties.  For example our 
test circuit (Figure 2.1) has 200 time simulation points, but in fact due to 
adaptive time step usage, Ngspice simulation has 213 simulation point, and Xyce 
simulation has 799 time points. This difference should be taken into account 
during postprocessing and comparing of simulation results. 

2.7 Other forms of simulation
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

You can use parameter sweep with AC, DC, and TRAN simulations. Parameter sweep 
features for ``spice4qucs`` will be considered in the Section 5.2.

Xyce supports Harmonic balance simulation. It will be described in the Section 
8.5. 

Ngspice supports the following additional simulations:
...
These simulations could be used via *Custom simulation* technique. See Chapter 
9 for details. 
